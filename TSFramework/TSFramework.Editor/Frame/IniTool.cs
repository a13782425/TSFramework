...
internal class IniTool
{
    ...
    public bool IsKeyExists(string SectionName, string Key)
    {
        ...
        {
            m_Sections.TryGetValue(SectionName, out Dictionary<string, string> value);
    ...
    public string ReadValue(string SectionName, string Key, string DefaultValue)
    {
        lock (m_Lock)
        {
            if (!m_Sections.TryGetValue(SectionName, out Dictionary<string, string> value))
            {
                return DefaultValue;
            }
            if (!value.TryGetValue(Key, out string value2))
            {
                return DefaultValue;
            }
            return value2;
    ...
    {
        string s = ReadValue(SectionName, Key, DefaultValue.ToString(CultureInfo.InvariantCulture));
        if (long.TryParse(s, NumberStyles.Any, CultureInfo.InvariantCulture, out long result))
        {
            return result;
        }
        return DefaultValue;
    ...
    {
        string s = ReadValue(SectionName, Key, DefaultValue.ToString(CultureInfo.InvariantCulture));
        if (DateTime.TryParse(s, CultureInfo.InvariantCulture, DateTimeStyles.AllowLeadingWhite | DateTimeStyles.AllowTrailingWhite | DateTimeStyles.AllowInnerWhite | DateTimeStyles.NoCurrentDateDefault | DateTimeStyles.AssumeLocal, out DateTime result))
        {
            return result;
        }
        return DefaultValue;
    ...
    {
        WriteValue(SectionName, Key, (!Value) ? "0" : "1");
    }
